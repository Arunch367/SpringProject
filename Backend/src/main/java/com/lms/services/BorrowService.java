...
//    }
    ...
    public Borrow requestBook(Borrow borrowBody) {
        ...
        // ✅ Only check total pending/accepted requests across all books
        List<Borrow> pendingRequests = getPendingBorrowRequestsByUserId(borrowBody.getUser().getId());
        if (pendingRequests.size() >= 2) {
            throw new MaximumBooksBorrowedException("You can only have 2 active borrow requests (pending or accepted) at a time.");
        }
        ...
        // Proceed with creating the borrow request
        Date currentDate = new java.sql.Date(System.currentTimeMillis());
        Calendar c = Calendar.getInstance();
        c.setTime(currentDate);
        c.add(Calendar.DATE, 7);
    ...
    public Borrow acceptRequest(Borrow borrow) {
        ...
        //just updating status of book reqeuest

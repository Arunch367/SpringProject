...
function MyAccount() {
  ...
  // admin update
  const tabItemStyle = (tabItem) => ({
    display: tabItem === tab ? "block" : "none",
    marginTop: "2rem",
    marginInline: "auto",
  });
  ...
  // get admin{id} data
  useEffect(() => {
    axios.get(`http://localhost:8080/admin/${adminId}`).then((res) => {
  ...
  const deleteAdmin = async () => {
    axios
      .delete("http://localhost:8080/admin/" + deleteId)
      .then((res) => {
        window.location.reload(false);
        setTab(TABS.delete);
      })
    ...
    <div class="main">
      ...
      <div style={{ marginTop: "20px" }}>
        <div id="setting-nav" class="topbara">
          ...
          <button onClick={() => setTab(TABS.changePassword)}>
            Change Password
          </button>
        ...
        <form style={tabItemStyle(TABS.editProfile)} id="editForm">
          ...
          <input
            ...
            autoComplete="off"
          />
          <button class="save-button" type="submit" onClick={updateProfile}>
            Save Changes
          </button>
        </form>
        <form id="addAdminForm" style={tabItemStyle(TABS.addAdmin)}>
          ...
          />
          <button class="save-button" type="submit" onClick={createAdmin}>
            Add Admin
          </button>
        </form>
        <form id="changePasswordForm" style={tabItemStyle(TABS.changePassword)}>
          ...
          />
          <button class="save-button" type="submit" onClick={changePassword}>
            Change Password
          </button>
        </form>
        ...
        <form id="deleteForm" style={tabItemStyle(TABS.delete)}>
          ...
          ></input>
          <button class="save-button" type="submit" onClick={deleteAdmin}>
            Delete Admin
          </button>
        </form>
        <br />

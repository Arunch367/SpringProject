...
function MyBorrowedBook() {
  ...
  useEffect(() => {
    axios
      .get(`http://localhost:8080/borrow/user/${userId}`)
      .then((response) => {
  ...
  const handleReturn = (borrwID, bookID, userID) => {
    setBookId(bookID);
    axios
      .put("http://localhost:8080/borrow/return", {
        borrowId: borrwID,
        bookId: bookID,
        userId: userID,
      })
      .then((res) => {
        ...
        axios
          ...
          .then((response) => {
            ...
            if (returnId !== null) {
              axios
                .put(`http://localhost:8080/requests_return/${returnId}`, {
                  user: { id: userID },
                  book: { id: bookID },
                })
                .then((response) => {
    ...
    axios
      ...
      .then((response) => {
        ...
        if (existingRating) {
          ...
          axios
            .put(`http://localhost:8080/ratings/${existingRating.id}`, {
              rating: rating,
              review: review,
              user: { id: userId },
              book: { id: bookId },
              ...
              <tr key={br.id}>
                ...
                <td>
                  {br.status === "ACCEPTED" ? (
                    <button
                      onClick={() =>
                        handleReturn(br.borrowId, br.book.id, userId)
                      }
                    >
                      Return
                    </button>
